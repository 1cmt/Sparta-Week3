...
internal class Store
{
    ...
    public void PurchaseMenu(string? prompt = null, Player player, Inventory inventory)
    {
        if (prompt != null)
        {
            Console.Clear();
        ...
        switch (keyInput)
        {
            ...
            default:
                ...
                {
                    PurchaseMenu("이미 구매한 아이템입니다.", player);
                }
                // 2 : 돈이 충분해서 살 수 있는 경우
                ...
                {
                    PurchaseMenu("Gold가 부족합니다.", player);
                }
                break;
        }
    }

    public void SellMenu(string? prompt = null, Player player, Inventory inventory)
    {
        if (prompt != null)
        {
            Console.Clear();
